...
enum encoding input_type_check( const char *input_path);
void vflag_print(int vflag, uint8_t * read_buffer, int getcodePoint, int moreByteToRead);
...
void codePointConvert(int getcodePoint, enum encoding output_encoding_enum, uint8_t * write_buff, int*ByteToWrite); // from codepoint to the require output file
// call the following 3 helper function 
void codePointToUTF8(int getcodePoint,  uint8_t * write_buffer, int *ByteToWrite);
void codePointToUTF16LE(int getcodePoint,  uint8_t * write_buffer, int *ByteToWrite);
void codePointToUTF16BE(int getcodePoint,  uint8_t * write_buffer, int *ByteToWrite);
// for utf16
...
int main (int argc, char*argv[]){
    ...
    if ( (input_encoding_enum =input_type_check(input_path))==INVALIDENCODING){
        print_usage();
        exit(EXIT_FAILURE);
    }
    ...
    while(filebytes_read = fread(read_buffer,1,1,input_file_pointer) == 1){ // if can sucessfully read the data 
        ...
        if(input_encoding_enum==UTF8){
            ...
            vflag_print(vflag, read_buffer, getcodePoint,moreByteToRead);
        }                                                         
        ...
        codePointConvert(getcodePoint, output_encoding_enum,write_buffer,ByteToWrite); // convert code point to the UTF8/UTF16LE/UTF16BE type output
...
void vflag_print(int vflag, uint8_t *read_buffer, int getcodePoint, int moreByteToRead){
    if(vflag==1){
        if(moreByteToRead==0){
            fprintf(stdout, "|    %c    |      1     |   U+%04x  |\n", *read_buffer, getcodePoint);
            fprintf(stdout, "+---------+------------+-----------+\n");
        }
         ...
         if(moreByteToRead==0){
          fprintf(stdout,"|    %c    |      1     |   U+%04x  |     %x   |\n", *read_buffer, getcodePoint,*read_buffer);
          fprintf(stdout,"+---------+------------+------------+----------+\n");
          }
         else if(moreByteToRead==1){
          fprintf(stdout,"|   NONE  |      2     |   U+%04x   |   0x%x%x |\n", getcodePoint,*read_buffer,*(read_buffer+1));
          fprintf(stdout,"+---------+------------+------------+----------+\n");
          }
         else if(moreByteToRead==2){
          fprintf(stdout,"|   NONE  |      3     |   U+%04x   |   0x%x%x%x |\n", getcodePoint,*read_buffer,*(read_buffer+1),*(read_buffer+2));
          fprintf(stdout,"+---------+------------+------------+----------+\n");
           }
         else if(moreByteToRead==3){
          fprintf(stdout,"|   NONE  |      4     |   U+%04x   |   0x%x%x%x%x |\n", getcodePoint,*read_buffer,*(read_buffer+1),*(read_buffer+2),*(read_buffer+3));
          fprintf(stdout,"+---------+------------+------------+----------+\n");
             }
...
void print_info(const char * input_path, const char *output_path, enum encoding input_encoding_enum, enum encoding output_encoding_enum){
    ...
    //struct stat sb;
    stat(input_path, &sb);
    printf("CSE320: Input: %s  %lu %lu %ld\n", input_path,sb.st_dev, sb.st_ino,sb.st_size);
...
enum encoding e_flag_command(int eflag, char*output_encoding){   
    if (eflag==0){
        printf("you have to have a -e command\n");
        return INVALIDEFLAG;
    }
    else if(eflag ==1 ){
        if (output_encoding==NULL){
            ...
            printf("not valid ENCODING format(too long)!\n");
            return INVALIDEFLAG;
        } 
            ...
            printf("not valid ENCODING format!\n");
            return INVALIDEFLAG;
        }
    }
}
...
int validate_file (const char * input_path, const char *output_path){
    ...
    if( (output_path != NULL) && ( strcmp(input_path,output_path)==0)){
        printf("output file should not be the same as the input file\n");
        return 0;
    }
...
enum encoding input_type_check( const char *input_path){
    ...
    if (read_value[0] == 0xFF && read_value[1] == 0XFE){
        ...
        if (read_value[2]==0xBF){
            ...
            printf("this is not a valid BOM\n");
            input_encoding= INVALIDENCODING; 
        }
    }
    else {
...
void getCodePointFromUFT8(uint8_t *read_buffer, int *moreToRead, int *codePoint){
    ...
    if ((*moreToRead)==0){
        ...
        *codePoint=Z;  // save the code point to the address 
        ...
        unsigned int A_p = A<<12;
        unsigned int B_p=B<<6;
        ...
        *codePoint=Z;
        ...
        unsigned int A_p=A<<18;
        unsigned int B_p=B<<12;
        unsigned int C_p=C<<6;
        unsigned int Z= (((A_p|B_p)|C_p)|D); // get the code point 
        *codePoint=Z;
    }
}
void codePointConvert(int getcodePoint, enum encoding output_encoding_enum, uint8_t * write_buffer, int*ByteToWrite){
    if(output_encoding_enum==UTF8){
        codePointToUTF8(getcodePoint,  write_buffer, ByteToWrite);
    }
    else if(output_encoding_enum==UTF16LE){
        codePointToUTF16LE(getcodePoint, write_buffer, ByteToWrite);
    }
    else if(output_encoding_enum==UTF16BE){
        codePointToUTF16BE(getcodePoint, write_buffer, ByteToWrite);
    }
}
...
void codePointToUTF16LE(int getcodePoint,  uint8_t * write_buffer, int *ByteToWrite){
    if ((getcodePoint>=0X0000 && getcodePoint<=0XD7FF)|| (getcodePoint>=0xE000 && getcodePoint<=0xFFFF)){
        ...
        *ByteToWrite=2;
        ...
        // get each byte       
        uint8_t A=(W1&0xFF00)>>8;
        uint8_t B=(W1&0xFF);
        uint8_t C=(W2&0XFF00)>>8;
        uint8_t D=(W2&0xFF);
        // little endian 
        write_buffer[0]=B;
        write_buffer[1]=A;
...
void getCodePointFromUFT16LE(uint8_t *read_buffer, int *moreToRead, int *codePoint){
    if((*moreToRead)==0){
        ...
        *codePoint = value;
        ...
        uint8_t byte2= *read_buffer;
        uint16_t w1= ((byte1<<8)&0xff00)|byte1; 
